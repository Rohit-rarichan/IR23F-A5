...
class NewContactDialog(tk.simpledialog.Dialog):
    ...
    def body(self, frame):
        ...
        self.password_label = tk.Label(frame, width=30, text="Username")
...
class MainApp(tk.Frame):
    ...
    def get_credentials(self):
        # Open a dialog to get username and password
        ld = LoginDialog(self.root, "Login", self.username, self.password)
        if ld.user and ld.pwd:
            self.username = ld.user
            self.password = ld.pwd
    ...
    def add_contact(self):
        new_contact = tk.simpledialog.askstring("Add Contact", "Enter new contact's username: ")
        if new_contact:
    ...
    def check_new(self):
        ...
        for sender, message in new_messages.items():
            self.publish(f"{sender} : {message}")
